...
var CardinalityCheck = require('./cardinality-check.js');
...
function StatsDClient(options) {
    ...
    this.cardinalityCheck = null;

    if (this.options.cardinalityCheck) {
        var opts = this.options.cardinalityCheck;
        this.cardinalityCheck = CardinalityCheck({
            statsdClient: this,
            intervalTime: opts.intervalTime,
            statName: opts.statName,
            maximumLength: opts.maximumLength
        });
        this.cardinalityCheck.bootstrap();
    }
}
...
StatsDClient.prototype.send = function send(message) {
    if (this.cardinalityCheck) {
        this.cardinalityCheck.addKey(message.name);
    }
    this._ephemeralSocket.send(message.toString());
};

StatsDClient.prototype.sendImmediate =
function sendImmediate(message, cb) {
    if (this.cardinalityCheck) {
        this.cardinalityCheck.addKey(message.name);
    }
