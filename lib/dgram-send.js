        ...
        if (!socket._sendQueue) {
            ...
            socket.once('listening', function onListening() {
                ...
                for (var i = 0; i < socket._sendQueue.length; i++) {
                    var args = [socket].concat(socket._sendQueue[i]);

                    dgramSend.apply(null, args);
                }
                socket._sendQueue = undefined;
            });
        }

        /* Because we are doing unsafe buffers we have to copy
           here.
        */
        var newBuffer = new Buffer(length - offset);
        buffer.copy(newBuffer, offset, 0, length - offset);

        socket._sendQueue.push([
            newBuffer, offset, length, port, address, callback
        ]);
        return;
    }
