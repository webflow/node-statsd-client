...
function ephemeralSocket(options) {
    ...
    // In the case socket.send is used without a callback and it results in an error,
    // instead of passing the error to the callback it will be thrown - resulting in
    // otherwise uncaught exceptions bubbling up into the application.
    self._onCatchSendError = opts.onCatchSendError || function() {};
