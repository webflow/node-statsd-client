...
EphemeralSocket.prototype._createSocket = function (callback) {
    ...
    this._socket.once('listening', function () {
        ...
        dns.lookup(that._hostname, function onDns(err, ip) {
            if (err) {
                return that._socket.emit('error', err);
            }
...
EphemeralSocket.prototype._send = function (data) {
    ...
    this._createSocket(function () {
        ...
        if (!that._socket._sendQueue ||
            that._socket._sendQueue.length < that.highWaterMark
        ) {
            that._socket.send(message, 0, message.length,
                that._port, that._ip);
        }
    });
};
