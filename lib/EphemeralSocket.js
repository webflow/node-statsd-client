...
function ephemeralSocket(options) {
    ...
    function onFlush(buf, offset, len) {
        self._writeToSocket(buf, offset, len);
    }
}
...
ephemeralSocket.prototype._writeString = function _writeString(str, cb) {
    ...
    this._writeToSocket(buf, 0, buf.length, cb);
};

/*
 * Send data.
 */
ephemeralSocket.prototype._writeToSocket = function _writeToSocket(buf, index, len, cb) {
    ...
    if (!this._socket._sendQueue ||
        ...
        this._socketSend(host, buf, index, len, cb);
    } else if (cb) {
...
ephemeralSocket.prototype._socketSend = function _socketSend(host, buf, index, len, cb) {
    var self = this;

    if (self._unsafeIPOnly) {
        dgramSend(self._socket, buf, index, len, self._port, host, cb);
    } else {
