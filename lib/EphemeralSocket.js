...
var dgramSend = require('./dgram-send.js');
...
function ephemeralSocket(options) {
    ...
    self._unsafeIPOnly = self.options.unsafeIPOnly;
    ...
    self._port = this.options.port;
    self._sharedBuffer = new Buffer(self._queue._blockSize + 500);

    // flush the packet queue
    function onFlush(str) {
        self._writeToSocket(str);
    }
}
...
ephemeralSocket.prototype.close = function () {
    ...
    if (this._queue._writePos > 0) {
        this._queue._sendPacket();
    }
...
ephemeralSocket.prototype._writeString = function _writeString(str, cb) {
    this._writeToSocket(str, cb);
};

/*
 * Send data.
 */
ephemeralSocket.prototype._writeToSocket = function _writeToSocket(str, cb) {
...
ephemeralSocket.prototype._socketSend = function _socketSend(host, str, cb) {
    var self = this;

    if (self._unsafeIPOnly) {
        var strLength = self._sharedBuffer.write(str);

        dgramSend(
            self._socket, self._sharedBuffer, 0, strLength,
            self._port, host, cb
        );
    } else {
        ...
        self._socket.send(buf, 0, buf.length, self._port, host, cb);
    }
};
